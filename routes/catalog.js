...
router.post('/add', function(req, res, next) {
    ...
    router.get('/cart', function(req, res, next) {
        if (!Array.isArray(req.session.cart) || !req.session.cart.length) {
            ...
            let query = "SELECT product_id, productname, prodimage, supplier_id, category_id, dimensions, prodprice, status FROM product WHERE product_id IN (" + req.session.cart + ") order by find_in_set(product_id, '" + req.session.cart + "');";
        ...
        db.query(insertquery,[req.session.customer_id],(err, result) => {
            if (err) {
                ...
                req.session.cart.forEach((cartitem, index) => { 
                    ...
                    let insertquery= "INSERT INTO orderdetail(order_id, product_id, saleprice, qty) VALUES (?, ?, (SELECT prodprice from product where product_id= " + cartitem+ "), ?)";
